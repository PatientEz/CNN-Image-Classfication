...
rootpath='E:\CV作业\MyWorkSpace\';
%%
addpath libsvm;  % 对应在设置路径中添加
addpath BOW;
addpath AdaBoost;
...
pg_opts.nclasses=length(pg_opts.classes);  % 类的数目

load(sprintf('%s',pg_opts.labels));  
pg_opts.nimages=size(labels,1);   % 图片数量

load(pg_opts.trainset);
load(pg_opts.testset);
pg_opts.ntraning =  length(find(trainset==1));  % 训练集数量
pg_opts.ntesting =  length(find(testset==1));   % 测试集数量
